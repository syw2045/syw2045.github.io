...
Value stream (V)가 주목하는 영역은 Jacobian 절대값 $$\left| \nabla_s \hat{V}(s) \right|$$을 계산하고

Advantage stream(A)가 주목하는 영역은 Jacobian 절대값 $$\left| \nabla_s \hat{V}(s, \argmax_{a'}\hat{A}(s,a');\theta) \right|$$를 계산한다.
