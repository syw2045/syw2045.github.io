...
하지만 모든 시점마다 계산하는 것은 비용이 많이 든다. 이를 해결하기 위해 stored buffer를 통해 공격에 대한 미래 state와 observation의 trajectories를 Planning하는데 사용한다.
...
앞서 설명한것처럼 initial attack을 선택하고 follow-up agents를 구분해 후속 타겟으로 정해 공격한다. 이렇게 학습을 반복하면서 $$Q^{tot}$$와 Planning Transformer를 업데이트한다. 
