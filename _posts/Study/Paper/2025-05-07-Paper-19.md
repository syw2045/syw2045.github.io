...
![alt text](../../../assets/images/Study/Paper/19/image.png)

1. Deep RL은 일반적으로 수많은 환경과의 상호작용이 필요하다. 하지만 <u>명시적인 보상 함수를 만들 수 없는 경우 인간이 직접 어떤 것이 더 좋은지 알려주는 방식이 필요</u>하다.
...
1. <u>정책 $$\pi$$가 환경과 상호작용하여 trajectory를 생성</u>한다. 정책 $$\pi$$의 파라미터는 기존의 RL 알고리즘을 사용하여 업데이트되고 예측된 보상 $$r_t = \hat{r}(o_t, a_t)$$ 의 합을 최대화하도록 학습한다.
2. 단계 1에서 진행된 <u>trajectory로부터 segments를 골라 인간이 비교</u>할 수 있게 한다.
3. 인간이 평가한 <u>비교 데이터를 사용하여 보상 함수 $$\hat{r}$$ 를 지도 학습방식으로 최적화</u>한다.
...
![alt text](<../../../assets/images/Study/Paper/19/image 6.png>)
...
![alt text](<../../../assets/images/Study/Paper/19/image 7.png>)
...
![alt text](<../../../assets/images/Study/Paper/19/image 9.png>)

Atari Game 환경

![alt text](<../../../assets/images/Study/Paper/19/image 10.png>)
...
이것은 인간의 선호도를 활용해서 Deep RL Agent를 효과적으로 학습할 수 있고 샘플 효율성 향상을 통해returns이 줄어드는 구간에 진입했다. 즉, 계산 비용이 이미 비전문가의 피드백 비용과 비슷한 수준에 도달했다.
